...
// Handle missing FIREBASE_PRIVATE_KEY gracefully
const rawPrivateKey = process.env.FIREBASE_PRIVATE_KEY;

let firebaseAdminConfig;
if (!rawPrivateKey) {
  // Warn and export a mock admin object if the key is missing
  console.warn('FIREBASE_PRIVATE_KEY is not set. Firebase Admin features will be disabled.');
  firebaseAdminConfig = null;
  // Optionally, export a mock or limited version of your service here
} else {
  firebaseAdminConfig = {
...
const serviceAccount = {
  ...
  clientEmail: process.env.FIREBASE_CLIENT_EMAIL || 'firebase-adminsdk-gdovy@universestay-8c0e4.iam.gserviceaccount.com',
  privateKey: process.env.FIREBASE_PRIVATE_KEY
  ? process.env.FIREBASE_PRIVATE_KEY.replace(/\\n/g, '\n')
  : (() => { throw new Error('FIREBASE_PRIVATE_KEY is not set'); })(),} as ServiceAccount;
